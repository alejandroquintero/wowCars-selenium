 ...
 */
package co.edu.uniandes.csw.concesionario.tests.postman.collections;
...
public class CarPrepare  {
   ...
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionCar.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,CarDetailDTO car,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(car);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(CarDetailDTO car) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:car.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(car, null).toString().concat("\",\""))); 
           }
